...
SELECT *, u.id as admin_id, CONCAT_WS(' ', u.first_name, u.second_name, u.last_name) AS admin_name,
    similarity(c.name, sqlc.arg('query')) AS sim
FROM city c
LEFT JOIN "user" u ON u.city_id = c.id
WHERE c.deleted_at IS NULL AND
    similarity(c.name, sqlc.arg('query')) > 0.2
