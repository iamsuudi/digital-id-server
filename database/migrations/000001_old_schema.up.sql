-- Enums
CREATE TYPE document_type AS ENUM ('ID', 'PASSPORT', 'DRIVING_LICENSE', 'NATIONAL_ID');
CREATE TYPE document_status AS ENUM ('PENDING', 'APPROVED', 'REJECTED');
CREATE TYPE payment_method AS ENUM ('CASH', 'MOBILE_MONEY', 'BANK_TRANSFER');
CREATE TYPE payment_status AS ENUM ('PENDING', 'APPROVED', 'REJECTED');
CREATE TYPE gender AS ENUM ('MALE', 'FEMALE', 'OTHER');
CREATE TYPE marital_status AS ENUM ('SINGLE', 'MARRIED', 'DIVORCED', 'WIDOWED');
CREATE TYPE religion AS ENUM ('CHRISTIAN', 'MUSLIM', 'HINDU', 'BUDDHIST', 'OTHER', 'NONE');
...
CREATE TABLE address (
    ...
    created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP(3),
    search_vector TSVECTOR GENERATED ALWAYS AS (to_tsvector('english', house_number || ' ' || district)) STORED,
...
CREATE TABLE resident (
    ...
    created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP(3),
    search_vector TSVECTOR GENERATED ALWAYS AS (to_tsvector('english', first_name || ' ' || second_name || ' ' || last_name)) STORED,
    FOREIGN KEY (address_id) REFERENCES address(id) ON DELETE SET NULL ON UPDATE CASCADE
);

CREATE TABLE biometric (
    id SERIAL PRIMARY KEY,
    resident_id SERIAL NOT NULL UNIQUE,
    fingerprint BYTEA,
...
CREATE TABLE idcard (
    ...
    created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP(3),
    search_vector TSVECTOR GENERATED ALWAYS AS (to_tsvector('english', number || ' ' || issue_place)) STORED,
    FOREIGN KEY (resident_id) REFERENCES resident(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE payment (
    ...
    created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP(3),
    search_vector TSVECTOR GENERATED ALWAYS AS (to_tsvector('english', description || ' ' || reference)) STORED,
    FOREIGN KEY (resident_id) REFERENCES resident(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE employment (
    ...
    created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMP(3),
    search_vector TSVECTOR GENERATED ALWAYS AS (to_tsvector('english', status || ' ' || COALESCE(occupation, '') || ' ' || COALESCE(employer_name, '') || ' ' || COALESCE(work_address, ''))) STORED,
    FOREIGN KEY (resident_id) REFERENCES resident(id) ON DELETE CASCADE ON UPDATE CASCADE
);
